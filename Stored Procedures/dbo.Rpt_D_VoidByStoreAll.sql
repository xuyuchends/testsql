...
CREATE Procedure [dbo].[Rpt_D_VoidByStoreAll]
	@BeginDate datetime,
	@EndDate datetime,
...
select * from ( SELECT s.storeName as StoreName'+@selSql+',
	SUM(isnull(CASE OI.RecordType WHEN ''VOID'' THEN OI.qty * OI.AdjustedPrice END,0)) as Void
...
set @sql +=' GROUP BY s.storeName '+@groupBySql+' ) a pivot (sum(Void) for StoreName in('+@colSql+')) b order by peroidID'
