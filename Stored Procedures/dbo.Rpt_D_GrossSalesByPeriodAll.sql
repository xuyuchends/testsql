...
CREATE proc [dbo].[Rpt_D_GrossSalesByPeriodAll]
(
	@BeginDate datetime,
	@EndDate datetime,
	@storeID int,
...
select *,dbo.f_GetOrderMealPeriodRpt(openTime,StoreID) as dayPart into #tempOrder from [dbo].[fnOrderTable]('''+ Convert(nvarchar,@BeginDate,120)+ ''','''+ Convert(nvarchar,@EndDate,120) + ''','''+convert(nvarchar(20),@storeID)+''')
...
select * from (Select o.dayPart as MealPeriod'+@selSql+',
isnull(SUM(OI.qty * OI.price),0) as GrossSales
From #tempOrderLineItem OI JOIN #tempOrder O ON OI.orderid = O.ID and OI.StoreID=O.StoreID  and OI.BusinessDate=O.BusinessDate 
