...
CREATE Procedure [dbo].[Rpt_D_TableSummaryByPcAll]
	@BeginDate datetime,
	@EndDate datetime,
...
set @jonTable=''
set @SalesTypeSql=''
select  @colName =dbo.fn_GetRevenueCenter(@storeID)
...
select * into #tempOrder from [dbo].[fnOrderTable]('''+ Convert(nvarchar,@BeginDate,120)+ ''','''+ Convert(nvarchar,@EndDate,120) + ''','''+@storeID+''')
	...
	select * from (	 Select o.RevenueCenter'+@selSql+',
	count(o.ID) as NumTables
	--sum(o.GuestCount) as NumGuest   
	From #tempOrder as o ' 
set @Sql+=' group by o.RevenueCenter '+@groupBySql+')a pivot (sum(NumTables) for RevenueCenter in('+@colName+')) b order by periodID'
--select @Sql
