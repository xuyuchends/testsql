...
CREATE Procedure [dbo].[Rpt_D_GuestSummaryByPeriodAll]
	@BeginDate datetime,
	@EndDate datetime,
...
select *,dbo.[f_GetOrderMealPeriodRpt](OpenTime,StoreID) SaleGroup into #tempOrder from [dbo].[fnOrderTable]('''+ Convert(nvarchar,@BeginDate,120)+ ''','''+ Convert(nvarchar,@EndDate,120) + ''','''+@storeID+''')
	...
	From #tempOrder as o  '
set @Sql+=' group by SaleGroup '+@groupBySql+')a pivot (sum(NumGuest) for MealPriod in('+@colName+')) b order by periodID'
