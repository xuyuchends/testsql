...
CREATE Procedure [dbo].[Rpt_D_GuestSummaryByPcAll]
	@BeginDate datetime,
	@EndDate datetime,
...
select * into #tempOrder from [dbo].[fnOrderTable]('''+ Convert(nvarchar,@BeginDate,120)+ ''','''+ Convert(nvarchar,@EndDate,120) + ''','''+@storeID+''')
	...
	From #tempOrder as o ' 
set @Sql+=' group by o.RevenueCenter '+@groupBySql+')a pivot (sum(NumGuest) for RevenueCenter in('+@colName+')) b order by periodID'
--select @Sql
