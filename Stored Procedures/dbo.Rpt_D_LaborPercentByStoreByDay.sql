...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[Rpt_D_LaborPercentByStoreByDay]
(
	@BeginDate datetime,
	@EndDate datetime,
...
convert(decimal(18,4) ,convert(decimal(18,2) ,SUM(ts.PayRate * case when ts.HoursWorked =0 then DATEDIFF( mi ,ts.TimeIn,GETDATE())/60.0 else ts.HoursWorked end)+SUM(ts.OT1HoursWorked * ts.OT1Payrate+ts.OT2HoursWorked  * ts.OT2Payrate))/      
(select SUM((Price-AdjustedPrice)*Qty) from 
OrderLineItem where BusinessDate=ts.BusinessDate and StoreID=ts.StoreID and status=''CLOSE''))*100    
...
inner join Store s on s.ID=ts.StoreID 
		...
		GROUP BY ts.BusinessDate,s.StoreName,ts.StoreID'
