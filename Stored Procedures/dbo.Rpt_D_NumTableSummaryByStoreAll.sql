...
CREATE proc [dbo].[Rpt_D_NumTableSummaryByStoreAll]
(
	@BeginDate datetime,
	@EndDate datetime,
...
From #tempOrder as o inner join Store as s ON o.StoreID = s.ID  Group by s.StoreName'+@groupBySql+') a pivot (sum(NumTables) for StoreName in('+@colSql+')) b order by periodID'
