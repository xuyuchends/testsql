...
CREATE proc [dbo].[Rpt_D_GrossSalesByProfitCenterAll]
(
...
select @colSql= dbo.[fn_GetRevenueCenter](@storeID)
...
select * from (Select o.RevenueCenter as RevenueCenter'+@selSql+',
isnull(SUM(OI.qty * OI.price),0) as GrossSales
From #tempOrderLineItem OI JOIN #tempOrder O ON OI.orderid = O.ID and OI.StoreID=O.StoreID   
LEFT JOIN MenuItem MI ON OI.itemid = MI.ID and o.StoreID=MI.StoreID  
Where si <> ''N/A''  AND MI.Category <> ''''    AND MI.Category IS NOT NULL  and MI.MIType NOT IN (''GC'', ''IHPYMNT'')    Group By o.RevenueCenter '+@groupBySql+' ) a pivot (sum(GrossSales) for RevenueCenter in('+@colSql+')) b order by periodID'
