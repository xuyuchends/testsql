...
CREATE proc [dbo].[Rpt_MenuItemByDateAll]
(
	@StoreId int,
	@BeginDate datetime,
	@EndDate datetime,
...
select *,dbo.[f_GetOrderMealPeriodRpt](OpenTime,StoreID) as daypart into #order from #tempOrder
 ...
 select o.BusinessDate, mi.ReportCategory Category ,oli.ItemID,mi.ReportDepartment Department,mi.ReportName MenuItem,SUM(Qty*oli.Price) as sales ,sum(Qty) as QuantitySold ,sum(Qty*oli.Price)/sum(Qty) as Avg from #tempOrderLineItem oli inner join #order o
...
where 1=1
'+@sqlwhere+'
group by mi.ReportDepartment,mi.ReportCategory ,mi.ReportName,oli.ItemID,o.BusinessDate  order by mi.ReportDepartment,mi.ReportCategory ,mi.ReportName
'
