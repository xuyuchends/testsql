...
-- =============================================
CREATE PROCEDURE [dbo].[GoogleChart_NumberByPaymentTypeAllSel]
	@storeID int
AS
BEGIN
	...
	delete from GoogleChartNumberByPaymentTypeAll where storeID=@storeID
	insert into GoogleChartNumberByPaymentTypeAll
		SELECT @storeID,
		pm.Name as Payment,    
		...
		COUNT(isnull(p.CheckID,0)) AS numPayment,'MONTH'  FROM  
		Payment as p 
	...
	insert into GoogleChartNumberByPaymentTypeAll
		...
		COUNT(isnull(p.CheckID,0)) AS numPayment,'DAY'  FROM 
		Payment as p 
		INNER JOIN PaymentMethod AS pm ON p.MethodID = pm.Name AND pm.StoreID = p.StoreID  
