...
-- =============================================
CREATE PROCEDURE [dbo].[GoogleChart_GuestSummaryByPeriodAllSel]
	@storeID int
AS
BEGIN
	...
	delete from GoogleChartGuestSummaryByPeriodAll where storeID=@storeID
	insert into GoogleChartGuestSummaryByPeriodAll
		Select @storeID,
		...
		sum(isnull(o.GuestCount,0)) as NumGuest,
		'MONTH'     
		From (select *,isnull((select Name from MealPeriod where StoreID=o.StoreID  
			...
			when [DayOfWeek]='SATURDAY' then 7 end)),MealPeriod) SaleGroup  from [Order] o 
			where BusinessDate between @monthBeginDate and @endDate and StoreID=@storeID and Status='Close') as o 
		group by SaleGroup ,convert(char(7),O.BusinessDate,120),datepart(m, o.BusinessDate)
		,DATEPART(YEAR,o.BusinessDate)
		
	insert into GoogleChartGuestSummaryByPeriodAll
		Select @storeID,
		o.SaleGroup MealPriod,
		...
		From (select *,isnull((select Name from MealPeriod where StoreID=o.StoreID  
			...
			when [DayOfWeek]='THURSDAY' then 5
			when [DayOfWeek]='FRIDAY' then 6
			when [DayOfWeek]='SATURDAY' then 7 end)),MealPeriod) SaleGroup  from [Order] o where BusinessDate between @dayBeginDate and @dayEndDate and StoreID=@storeID and Status='Close') as o   
