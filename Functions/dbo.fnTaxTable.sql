...
CREATE function [dbo].[fnTaxTable]
(
	@BeginDate datetime,
	@EndDate datetime,
...
begin
	...
	FROM [Tax] where [BusinessDate] between @historyDate and @EndDate  and storeID in (select * from dbo.f_split(@StoreID,','))
	and status='VALID'

	insert into @resultTable([StoreID],[TaxCategoryID],[CheckID],[OrderID],[Category],[TaxAmt],[TaxOrderAmt],[BusinessDate],[LastUpdate])
	SELECT [StoreID],[TaxCategoryID],[CheckID],[OrderID],[Category],[TaxAmt],[TaxOrderAmt],[BusinessDate],[LastUpdate]
	FROM [TaxArchive] where [BusinessDate] between @BeginDate and @historyDate and storeID in (select * from dbo.f_split(@StoreID,','))
	and status='VALID'
...
begin
	insert into @resultTable([StoreID],[TaxCategoryID],[CheckID],[OrderID],[Category],[TaxAmt],[TaxOrderAmt],[BusinessDate],[LastUpdate])
	SELECT [StoreID],[TaxCategoryID],[CheckID],[OrderID],[Category],[TaxAmt],[TaxOrderAmt],[BusinessDate],[LastUpdate]
	FROM [Tax] where [BusinessDate] between @BeginDate and @EndDate and storeID in (select * from dbo.f_split(@StoreID,','))
	and status='VALID'
end
...
begin
	insert into @resultTable([StoreID],[TaxCategoryID],[CheckID],[OrderID],[Category],[TaxAmt],[TaxOrderAmt],[BusinessDate],[LastUpdate])
	SELECT [StoreID],[TaxCategoryID],[CheckID],[OrderID],[Category],[TaxAmt],[TaxOrderAmt],[BusinessDate],[LastUpdate]
	FROM [TaxArchive] where [BusinessDate] between @BeginDate and @EndDate and storeID in (select * from dbo.f_split(@StoreID,','))
	and status='VALID'
end
